...
public class Grid {
	...
	public Grid (int cellCount, double minX, double maxX, double minY, double maxY) {
		// initialize 
		topLevelCells = new Cell[cellCount][cellCount];//顶层是由N N个cell组成的
		
		// fill with cells
		double xIncrement = (maxX-minX)/((double) cellCount); //每个网格的边长
		double yIncrement = (maxY-minY)/((double) cellCount);//
		for (int i = 0; i < cellCount; i++) {
			for (int j = 0; j < cellCount; j++) {
				topLevelCells[i][j] = new Cell(minX+xIncrement*i, minY+yIncrement*j,
						xIncrement, yIncrement, ""+i+","+j);//每个顶层网格
			}
		}
		...
		for (int i = 0; i < this.getCells().size(); i++) {
			posInListForm.put(this.getCells().get(i), i); //get获取getCells里的元素 建立hash //添加键值对(key-value)可以使用 put() 方法:
		}
	}
	...
	public List<Cell> getCells() {
		...
		for (int i = 0; i < this.getN(); i++) {
			for (int j = 0; j < this.getN(); j++) {
				tbR.add(topLevelCells[i][j]);//创建出i行j列的矩阵
			}
		}
		return tbR;
	}
	...
	// return the list of cells on the shortest path between start cell and end cell 返回起始单元格和结束单元格之间最短路径上的单元格列表
	public List<Cell> giveInterpolatedRoute (Cell start, Cell end) {
		...
		if (startfound == false || endfound == false) {
			System.out.println("cells not found, cannot interpolate");//插入的意思
			return null;
		}
